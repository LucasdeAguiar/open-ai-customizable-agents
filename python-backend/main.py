...
# Configure logging para arquivo e console
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s %(levelname)s %(message)s",
    handlers=[
        logging.FileHandler("agent_metrics.log"),
...
guardrail_agent = Agent(
    ...
    instructions=(
        ...
        "but if the response is non-conversational, it must be somewhat related to airline travel or HR topics. "
...
def hr_instructions(
    ...
    return (
        ...
        "You are an HR Agent. You answer questions about company benefits, holidays, and HR policies. "
        "If the question is not about HR, transfer back to the triage agent."
    )

@function_tool(
    name_override="hr_policy_lookup",
    description_override="Lookup HR policies and benefits."
)
async def hr_policy_lookup(question: str) -> str:
    q = question.lower()
    if "benefit" in q or "plano" in q:
        return "Our company offers health, dental, and meal benefits."
    if "holiday" in q or "feriado" in q:
        return "The company observes all national holidays and offers 30 days of paid vacation."
    return "Please specify your HR-related question."

hr_agent = Agent[AirlineAgentContext](
    name="HR Agent",
    model="gpt-4.1",
    handoff_description="An agent that answers HR-related questions.",
    instructions=hr_instructions,
    tools=[hr_policy_lookup],
...
hr_agent.handoffs.append(triage_agent)
